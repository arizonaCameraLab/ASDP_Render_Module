...
public:
  ...
  std::string Draw(const char* text, float x, float y, float red, float green, float blue);
...
std::string RenderText::Impl::Draw(const char* text, float x, float y, float red, float green, float blue)
{
  float sx = text_width();
