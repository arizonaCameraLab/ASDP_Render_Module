...
#else
  extern "C" int vrpn_gettimeofday(struct timeval *tp,
                                            void *tzp = NULL);
#endif
...
extern unsigned long vrpn_TimevalDuration(struct timeval endT,
                                                   struct timeval startT);

/// @brief Return the number of seconds between startT and endT as a
/// floating-point value.
extern double vrpn_TimevalDurationSeconds(struct timeval endT,
                                                   struct timeval startT);

extern bool vrpn_TimevalGreater(const struct timeval &tv1,
                                         const struct timeval &tv2);
extern bool vrpn_TimevalEqual(const struct timeval &tv1,
                                       const struct timeval &tv2);

extern double vrpn_TimevalMsecs(const struct timeval &tv1);

extern struct timeval vrpn_MsecsTimeval(const double dMsecs);
extern void vrpn_SleepMsecs(double dMilliSecs);
...
extern vrpn_float64 vrpn_htond(vrpn_float64 d);
extern vrpn_float64 vrpn_ntohd(vrpn_float64 d);
