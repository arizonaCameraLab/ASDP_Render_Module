...
public:
  ...
  virtual bool  read_pixel(int x, int y, uint8_t &result, unsigned rgb = 0) const
  { double double_pix;
    bool err = read_pixel(x,y, double_pix, rgb);
    result = static_cast<uint8_t>(double_pix);
    return err;
  }
  virtual bool  read_pixel(int x, int y, uint16_t&result, unsigned rgb = 0) const
  { double double_pix;
    bool err = read_pixel(x,y, double_pix, rgb);
    result = static_cast<uint16_t>(double_pix);
    return err;
  }
...
public:
  ...
  virtual bool	get_pixel_from_memory(unsigned X, unsigned Y, uint8_t&val, int RGB = 0) const = 0;
  virtual bool	get_pixel_from_memory(unsigned X, unsigned Y, uint16_t &val, int RGB = 0) const = 0;
